...
consle.log(alexAge);
consle.log(alexMembershipID);
consle.log(isRunningOnTreadmill);
...
const GroupWeightTrainingClasses = 500;
const PersonWeightTrainingClass = 1500;
const AlexBudjetThisMonth = 3000;
...
const  AlexBudget = AlexBudjetThisMonth - ( GroupWeightTrainingClasses* 1 + YogaGroupClasses*3 +  PersonWeightTrainingClass*1)
...
const totalpackageClasees =14;
const totalpackagePrice = 2520;
const CourseTime = 50;
...
let isAlexWaiting = true; // Alex 是否在等待中 
let isRedLight = true; // 是否為紅燈 
let canCrossover = false; // 是否可通行 

let waitingMotorcycles = 8;// 機車數量 
...
let b = 0; // 宣告了一個 b 的變數，並賦予了一個 0 的數字型別 (b = 8)
...
let numberArr1 = [5, 10, 15]; //宣告一個變數 numberArr1，賦予陣列型別型別  [5, 10, 15]
let numberArr2 = numberArr1;  //宣告一個變數 numberArr2，將 numberArr1 陣列陣列型別  [5, 10, 15] 賦予給 numberArr2， numberArr2和 numberArr1 指向同一個記憶體位置
numberArr2.push(20);  增一數字型別元素 20 到 numberArr2 陣列，numberArr1也同樣會改變
numberArr2 = [25, 30, 35]; 重新賦予numberArr2陣列型別型別  [5, 10, 15],numberArr指向新的記憶體位置
