...
var l = '8';
// 請文字解釋為什麼
console.log(k * l > 21); true //乘法時，判斷 l可轉為數字，把l 轉為數字與k 相乘
...
{
  coachBonus = baseBonus + 0.2*coachBonus;
  
}else if(100000 <coachIncome =<300000)
{
  coachBonus = baseBonus + 0.15*coachBonus;
}else
{
    coachBonus = baseBonus + 0.1*coachBonus;
}
...
// if (performanceData.coaches[0].performance <target ) {
//   // 請填寫第一位教練業績增長程式碼，使用 +=
   performanceData.coaches[0].performance +=  target - performanceData.coaches[0].performance
// }

// 練習：第二位教練（可將下方程式碼註解移除，完成答題）

// if performanceData.coaches10].performance <target) {
//   // 請填寫第二位教練業績增長程式碼，使用 +=
   performanceData.coaches[1].performance +=  target - performanceData.coaches[1].performance
// }
...
exerciseRecords.forEach(items => {
  if (items.duration >= 30 && items.heartRate >= 130) {
